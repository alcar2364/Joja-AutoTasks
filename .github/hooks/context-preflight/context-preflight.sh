...
if echo "$PROMPT" | grep -qiE '(create|new file|new module|refactor|reorganize|folder|structure|rename|move file)'; then
  echo "[context-preflight] Load: .local/Agents/Instructions/context-engineering.instructions.md"
  echo "[context-preflight] Load: .local/Agents/Instructions/csharp-style-contract.instructions.md"
fi

if echo "$PROMPT" | grep -qiE '(workflow|github actions|ci/cd|pipeline|\.github/workflows|yaml)'; then
  echo "[context-preflight] Load: .local/Agents/Instructions/github-actions-ci-cd-best-practices.instructions.md"
fi

if echo "$PROMPT" | grep -qiE '(plan|planning|spec|design guide|architecture decision|adr)'; then
  echo "[context-preflight] Load: .local/Agents/Instructions/workspace-contracts.instructions.md"
fi
