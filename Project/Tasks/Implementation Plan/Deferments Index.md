...
| DEF-031      | Evaluate renaming `DayKey` to `DateKey` while preserving canonical `Year{N}-{Season}{D}` key format and contracts.                                     | Phase 3       | Phase 3 checklist, Deferred Items (`DayKey` to `DateKey` naming request)  | Phase 6          | Active | TBD   | Naming-only clarity proposal; no format change requested   |
