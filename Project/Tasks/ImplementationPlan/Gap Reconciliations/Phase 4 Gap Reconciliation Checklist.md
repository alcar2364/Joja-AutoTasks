...
- [x] Step 2 — Create `ToastEvent` sealed class (`file:State/ToastEvent.cs`)
...
- [x] Step 3 — Create `DeadlineStoredFields` sealed class (`file:State/Models/DeadlineStoredFields.cs`)
...
- [x] Step 4 — Create `DeadlineFields` sealed class (`file:State/Models/DeadlineFields.cs`)
...
- [x] Step 5 — Create `DeadlineEvaluator` static class (`file:State/DayBoundary/DeadlineEvaluator.cs`)
...
- [x] Step 6 — Add bootstrap guard policy seam
...
- [x] Step 7 — Update `CompleteTaskCommand` (`file:State/Commands/CompleteTaskCommand.cs`)
...
- [x] Step 8 — Update `TaskRecord` (`file:State/Models/TaskRecord.cs`)
...
- [x] Step 9 — Update `TaskView` (`file:State/Models/TaskView.cs`)
...
- [x] Step 10 — Update `SnapshotProjector` (`file:State/SnapshotProjector.cs`)
...
- [x] Step 11 — Update `StateStore` (`file:State/StateStore.cs`)
...
- [x] **11b — Add `ToastRequested` event:**

```
public event Action<ToastEvent>? ToastRequested;
```

- [x] **11c — Add `internal void SetBootstrapGuardPolicy(BootstrapGuardPolicy
policy)` for test seam.**

- [x] **11d — Add `internal void InitializeTimeContext(DayKey currentDay, int currentTime)`:**
...
- [x] **11e — Update `OnDayStarted` signature to `internal void
...
- [x] **11f — Add `internal void OnTimeChanged(DayKey currentDay, int currentTime)`:**
...
- [x] **11i — Add bootstrap guard helper method** `private void GuardTimeContextInitialized()`:
...
- [x] **11j — Update `Handle()` for `CompleteTaskCommand` toast emission:**

In the `switch` block, replace the `CompleteTaskCommand` case with:

- [x] Capture `priorStatus` by calling
...
- [x] Call `_completeTaskHandler.Handle(completeTaskCommand, _stateContainer)`.
- [x] After handler, if `_stateContainer.Version != priorVersion` (state
      changed) AND `priorStatus == TaskStatus.Incomplete` AND
      `completeTaskCommand.IsPlayerInitiated == false`:
  - [x] Retrieve the updated record to get its `Title`.
  - [x] Fire `ToastRequested?.Invoke(new
ToastEvent(ToastType.TaskAutoCompleted, title))`.
- [x] Then fall through to the existing version-check that fires `SnapshotChanged`.

The `SnapshotChanged` invocation must pass `_currentDayKey` and `_currentTime` to `SnapshotProjector.Project`.

- [x] **11k — Update all `SnapshotProjector.Project(...)` call sites in
      `StateStore`** to pass `_currentDayKey` and `_currentTime`. There are two:
      inside `Handle()` and inside `OnDayStarted()`.

---

- [x] Step 12 — Add `DispatchTimeChanged` to `IEventDispatcher` and `EventDispatcher`
...
- [x] Step 13 — Update `LifecycleCoordinator` (`file:Lifecycle/LifecycleCoordinator.cs`)

- [x] **13a — Add session-active tracking field:**
- `private bool _sessionActive;`

- [x] **13b — Update `HandleSaveLoaded()`:**
- Set `_sessionActive = true`.
...
- [x] **13d — Update `HandleReturnedToTitle()`:**
- Set `_sessionActive = false`.
- Keep existing dispatcher and store calls.

- [x] **13e — Add `internal void HandleTimeChanged(DayKey currentDay, int currentTime)`:**
- If `!_sessionActive`: return (session gate).
...
- [x] Step 14 — Update `ModEntry` (`file:ModEntry.cs`)

- [x] **14a — Register `TimeChanged` hook** in `Entry()`:

```csharp
helper.Events.GameLoop.TimeChanged += OnTimeChanged;
```

- [x] **14b — Add `OnTimeChanged` handler:**
...
- [x] **14d — Update `OnDayStarted`:**
- Extract `DayKey` and `currentTime` from game state.
- Forward to `_runtime.LifecycleCoordinator.HandleDayStarted(newDay, currentTime)`.

- [x] **14e — Initialize `UiToastSubscriptionManager`** alongside `UiSnapshotSubscriptionManager`:
...
- [x] Step 15 — Create `UiToastSubscriptionManager` (`file:UI/UiToastSubscriptionManager.cs`)
...
- [x] Step 16 — Update `DayKeyFactory` usage in `ModEntry`
