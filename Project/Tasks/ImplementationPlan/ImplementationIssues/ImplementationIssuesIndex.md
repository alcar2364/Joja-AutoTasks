...
| 103 | Deferment | Enforce sequential RuleId generation once actual RuleId generation flow exists. | Phase 2 | Phase 2 checklist guardrails/final gate (RuleId generation defer) | Phase 6 | Scheduled | Low |  | Deferred until RuleId generation exists |
...
| 105 | Deferment | Add deterministic task-type ordering/comparer test coverage with comparer implementation. | Phase 2 | Phase 2 checklist, Step 7F (comparer tests defer) | Phase 5+ | Scheduled | Low |  |  |
...
| 107 | Deferment | Wire ViewModels to subscribe to SnapshotChanged so snapshot updates propagate into UI state. | Phase 3 | Phase 3 checklist, Deferred Items (Phase 4 ViewModels) | Phase 4 | Scheduled | Low |  |  |
...
| 113 | Deferment | Implement task-type ordering/comparison behavior in runtime usage paths. | Phase 3 | Phase 3 checklist, Deferred Items (Phase 5+ Generators/Engine) | Phase 5+ | Scheduled | Low |  |  |
...
| 115 | Deferment | Implement rule-driven command generation from evaluated Task Builder rules. | Phase 3 | Phase 3 checklist, Deferred Items (Phase 6 Rule Engine) | Phase 6 | Scheduled | Low |  |  |
...
| 117 | Deferment | Persist manual task counter across sessions to preserve deterministic manual TaskId continuity. | Phase 3 | Phase 3 checklist, Deferred Items (Phase 7 Persistence) | Phase 7 | Scheduled | Low |  |  |
...
| 125 | Deferment | Add multiplayer synchronization for shared task state consistency. | Phase 3 | Phase 3 checklist, Deferred Items (V2) | V2+ | Scheduled | Low |  | May be V2 scope |
...
| 127 | Deferment | Evaluate DayKey internal representation by storing year/season/day components alongside canonical string for comparison and calculation ergonomics. | Phase 3 | Phase 3 checklist, Deferred Items (open issue: DayKey representation) | Phase 6 | Scheduled | Low |  | Requires design decision on storage-versus-parse tradeoff |
| 131 | Deferment | Decide the permanent home for `ManualTaskCounter` after generator/UI service boundaries are established. | Phase 3 | Phase 3 checklist, Deferred Items (`ManualTaskCounter` location decision) | Phase 5+ | Scheduled | Low |  | Keep `State/Models/` as working location until decision |
| 132 | Deferment | Evaluate renaming `DayKey` to `DateKey` while preserving canonical `Year{N}-{Season}{D}` key format and contracts. | Phase 3 | Phase 3 checklist, Deferred Items (`DayKey` to `DateKey` naming request) | Phase 6 | Scheduled | Low |  | Naming-only clarity proposal; no format change requested |
