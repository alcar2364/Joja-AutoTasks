...
| 126 | Review follow-up | Evaluate whether TaskRecord should remain a value type or become a reference type based on mutation patterns and state dictionary semantics. | Phase 3 | Phase 3 checklist, Deferred Items (open issue: TaskRecord type decision) | Phase 5 | Open | Medium |  | Decision required before deeper optimization/refactor work |
| 100 | Deferment | Implement localization/translation behavior that changes runtime task content and flow, not just documentation. | Phase 2 | Phase 2 checklist, Step 4D (translation-impacting implementation defer) | Phase 3+ | Open | Low |  |  |
